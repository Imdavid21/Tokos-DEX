...
for(const name of(await readdir(dir)).filter(n=>n.endsWith(".sql")).sort()){
 ...
 if(name.includes("timescale")){
  const ext=await db.query("SELECT EXISTS(SELECT 1 FROM pg_extension WHERE extname='timescaledb') ok");
  if(!ext.rows[0]?.ok){console.log(`skipped ${name} (timescaledb unavailable)`);continue;}
 }
 const sql=await readFile(resolve(dir,name),"utf8"),c=await db.connect();
