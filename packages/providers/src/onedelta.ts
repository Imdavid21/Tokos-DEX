...
const lenderInfoSchema=z.object({key:z.string(),name:z.string().nullable().optional(),logoURI:z.string().nullable().optional()}).passthrough();
const lenderSchema=z.object({chainId:z.string(),lenderInfo:lenderInfoSchema,tvlUsd:z.number().nullable().optional(),lastFetched:z.number().nullable().optional()}).passthrough();
...
const latestMarketSchema=z.object({
 ...
 underlyingInfo:z.object({asset:assetMetaSchema.nullable().optional()}).passthrough().nullable().optional(),underlying:assetMetaSchema.nullable().optional(),terms:z.array(z.unknown()).nullable().optional().transform(v=>v??[]),flags:z.record(z.string(),z.unknown()).optional(),termSheet:z.unknown().optional(),rewards:z.unknown().optional()
}).passthrough();
const latestItemSchema=z.object({chainId:z.string(),lenderInfo:lenderInfoSchema,lastFetched:z.number(),totalDepositsUsd:z.number().nullable().optional(),totalDebtUsd:z.number().nullable().optional(),tvlUsd:z.number().nullable().optional(),fixedTerm:z.object({model:z.string().optional(),maturity:z.number().optional()}).passthrough().nullable().optional(),markets:z.array(latestMarketSchema)}).passthrough();
