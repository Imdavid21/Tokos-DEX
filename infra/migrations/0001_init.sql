...
DO $do$ BEGIN IF EXISTS(SELECT 1 FROM pg_available_extensions WHERE name='timescaledb') THEN CREATE EXTENSION IF NOT EXISTS timescaledb; END IF; END $do$;
...
DO $do$ BEGIN IF EXISTS(SELECT 1 FROM pg_extension WHERE extname='timescaledb') THEN PERFORM create_hypertable('market_snapshots',by_range('observed_at'),if_not_exists=>TRUE); END IF; END $do$;
...
DO $do$ BEGIN IF EXISTS(SELECT 1 FROM pg_extension WHERE extname='timescaledb') THEN PERFORM create_hypertable('execution_depth_snapshots',by_range('observed_at'),if_not_exists=>TRUE); END IF; END $do$;
...
DO $do$ BEGIN IF EXISTS(SELECT 1 FROM pg_extension WHERE extname='timescaledb') THEN PERFORM create_hypertable('basis_snapshots',by_range('observed_at'),if_not_exists=>TRUE); END IF; END $do$;
