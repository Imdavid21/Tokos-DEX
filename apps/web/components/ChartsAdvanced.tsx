...
export function BubbleChart({points,height=330}:{points:Array<{x:number;y:number;size:number;label:string;group?:string;detail?:string}>;height?:number}){
  ...
  const ref=useChart(()=>{const colors=palette(),groups=[...new Set(points.map(p=>p.group??p.label))];return{animationDuration:220,grid:{left:68,right:28,top:28,bottom:46},tooltip:{confine:true,formatter:(p:unknown)=>{const raw=(p as{data:{value?:[number,number,number,string,string,string]}|[number,number,number,string,string,string]}).data,d=Array.isArray(raw)?raw:raw.value;if(!d)return"";return `${d[3]}${d[4]?` · ${d[4]}`:""}<br/>Yield ${(d[1]*100).toFixed(2)}%<br/>Liquidity ${fmt("usd",d[2])}${d[5]?`<br/>${d[5]}`:""}`}},xAxis:{type:"value",name:"Days",nameTextStyle:{color:css("--muted"),fontSize:11},axisLabel:{color:css("--muted"),fontSize:11},splitLine:{lineStyle:{color:css("--border")}}},yAxis:{type:useLog?("log" as const):("value" as const),logBase:10,name:useLog?"Yield · log scale":undefined,nameTextStyle:{color:css("--muted"),fontSize:10},axisLabel:{color:css("--muted"),fontSize:11,formatter:(v:number)=>fmt("percent",v)},splitLine:{lineStyle:{color:css("--border")}}},dataZoom:[{type:"inside"}],series:[{type:"scatter",data:points.map(p=>({value:[p.x,p.y,p.size,p.label,p.group??"",p.detail??""],itemStyle:{color:colorAt(colors,Math.max(0,groups.indexOf(p.group??p.label))),opacity:.74}})),symbolSize:(v:unknown)=>{const d=v as number[];return Math.max(7,Math.min(30,7+23*Math.sqrt((d[2]??0)/maxSize)))},emphasis:{focus:"self",itemStyle:{opacity:1}}}]};},[points,useLog]);
