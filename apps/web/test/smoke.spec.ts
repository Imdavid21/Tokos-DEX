...
test("research flow works with live normalized data",async({page,request})=>{
 ...
 if(h1&&h2){const id1=decodeURIComponent(h1.split("/market/")[1]??""),id2=decodeURIComponent(h2.split("/market/")[1]??"");await page.goto(`/compare?markets=${encodeURIComponent(id1)},${encodeURIComponent(id2)}`);await expect(page.getByRole("heading",{name:"Compare",exact:true})).toBeVisible()}
 ...
 for(const [route,title] of [["/yield-curve?asset=usdc&notional=100000","Yield Curve"],["/basis","Basis"],["/execution","Liquidity"]] as const){await page.goto(route);await expect(page.getByRole("heading",{name:title,exact:true})).toBeVisible()}
