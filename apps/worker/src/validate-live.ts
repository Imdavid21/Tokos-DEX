...
async function ingestOneDeltaSample(){
 const{oneDelta}=services(env),chains=await oneDelta.chains(),selected=chains.find(c=>c.chainId==="1")??chains[0];if(selected===undefined)throw new Error("live validation failed: 1delta returned no chains");const chain=selected;
 const lenders=await oneDelta.lenders([chain.chainId]),keys=[...new Set(lenders.map(r=>r.lenderInfo.key))].slice(0,12);let markets=0,successfulLenders=0;const failures:Array<{key:string;error:string}>=[];
 for(const key of keys){try{const response=await oneDelta.latest([chain.chainId],[key],{terms:"digest",attempts:2});await repo.insertProviderObservation({provider:"1delta",endpoint:"/data/lending/latest",requestFingerprint:`live-validation:${chain.chainId}:${key}`,httpStatus:response.status,success:true,payload:response.raw,payloadHash:payloadHash(response.raw)});let lenderMarkets=0;for(const item of response.parsed.data.items)for(const source of item.markets){const n=normalizeOneDeltaMarket(item,source,chain.name);await repo.transaction(async c=>{await repo.upsertChain(n.chain,c);await repo.upsertAsset(n.asset,c);await repo.upsertProtocol(n.protocol,c);await repo.upsertMarket(n.market,{termSheet:source.termSheet??null},c);await repo.insertSnapshot(n.snapshot,c)});markets++;lenderMarkets++;}if(lenderMarkets>0)successfulLenders++;if(successfulLenders>=4&&markets>=8)break;}catch(error){const message=error instanceof Error?error.message:String(error);failures.push({key,error:message});const payload={error:message,lender:key};await repo.insertProviderObservation({provider:"1delta",endpoint:"/data/lending/latest",requestFingerprint:`live-validation:${chain.chainId}:${key}`,httpStatus:0,success:false,payload,payloadHash:payloadHash(payload),errorCode:null});}}
 if(markets<1)fail(`1delta sample produced no normalized markets; failures=${JSON.stringify(failures.slice(0,3))}`);return{chain:chain.chainId,lendersTried:successfulLenders+failures.length,successfulLenders,failedLenders:failures.map(x=>x.key),markets};
}
