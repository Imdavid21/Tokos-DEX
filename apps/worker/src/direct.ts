...
void bootstrap().then(async()=>{for(const task of tasks)setInterval(()=>void execute(task),task.every);await logReadiness("bootstrap");const latest=tasks.find(x=>x.name==="onedelta-latest");if(latest)await execute(latest);const depth=tasks.find(x=>x.name==="onedelta-depth");if(depth)await execute(depth);await logReadiness("lending-refresh")});
