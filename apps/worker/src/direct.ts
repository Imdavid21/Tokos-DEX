...
void bootstrap().then(async()=>{for(const task of tasks)setInterval(()=>void execute(task),task.every);await logReadiness("bootstrap");const latest=tasks.find(x=>x.name==="onedelta-latest");if(latest)await execute(latest);const basis=tasks.find(x=>x.name==="cross-basis");if(basis)await execute(basis);await logReadiness("lending-refresh");await probeApiReadiness();const depth=tasks.find(x=>x.name==="onedelta-depth");if(depth)void execute(depth)});
