...
const MIME = {
  ...
  '.svg': 'image/svg+xml', '.json': 'application/json; charset=utf-8', '.ico': 'image/x-icon', '.webp': 'image/webp',
}

const buckets = new Map()
function rateLimited(ip) {
  const now = Date.now(), windowMs = 60_000, max = 90, current = buckets.get(ip)
...
function json(res, status, body) {
  ...
  res.writeHead(status, securityHeaders({ 'Content-Type': 'application/json; charset=utf-8', 'Cache-Control': 'no-store', 'Content-Length': Buffer.byteLength(payload) }))
  res.end(payload)
}
function getParams(url, mode) {
  const chainId = url.searchParams.get('chainId'), tokenIn = url.searchParams.get('tokenIn'), tokenOut = url.searchParams.get('tokenOut')
  const amount = url.searchParams.get('amount'), slippage = url.searchParams.get('slippage') || '50', account = url.searchParams.get('account')
...
async function directBenchmark(res, url) {
  ...
  try {
    const upstream = await fetch(DIRECT_QUOTE_URL, {
      method: 'POST', headers: { Accept: 'application/json', 'Content-Type': 'application/json', 'x-api-key': DIRECT_MARKET_API_KEY, 'x-universal-router-version': '2.0', 'x-erc20eth-enabled': 'true' },
      body: JSON.stringify({ type: 'EXACT_INPUT', amount: parsed.amount, tokenInChainId: Number(parsed.chainId), tokenOutChainId: Number(parsed.chainId), tokenIn: parsed.tokenIn, tokenOut: parsed.tokenOut, swapper: parsed.account, slippageTolerance: Number(parsed.slippage) / 100 }),
      signal: AbortSignal.timeout(10_000),
    })
    const body = await upstream.json().catch(() => null)
    if (!upstream.ok || !body) return json(res, 502, { success: false, error: { message: 'Direct benchmark is unavailable' } })
...
async function gt(path) {
  const r = await fetch(`${GT_BASE}${path}`, { headers: gtHeaders(), signal: AbortSignal.timeout(8_000) })
  const body = await r.json().catch(() => null)
  if (!r.ok || !body) throw new Error('market data unavailable')
  return body
}
async function tokenData(res, url) {
  const address = url.searchParams.get('address')
  if (!address || !ADDRESS.test(address)) return json(res, 400, { success: false, error: { message: 'Invalid token address' } })
  const marketAddress = address.toLowerCase() === ZERO ? WETH : address
  try {
    const body = await gt(`/networks/eth/tokens/${marketAddress}/info`)
    const a = body?.data?.attributes || {}
    return json(res, 200, { success: true, data: { address, symbol: a.symbol || null, name: a.name || null, image: a.image_url || null, coingeckoCoinId: a.coingecko_coin_id || null, websites: a.websites || [], description: a.description || null } })
  } catch {
    return json(res, 200, { success: true, data: { address, symbol: address.toLowerCase() === ZERO ? 'ETH' : null, name: address.toLowerCase() === ZERO ? 'Ether' : null, image: null } })
  }
}
async function marketData(res, url) {
  const address = url.searchParams.get('address'), daysRaw = url.searchParams.get('days') || '1'
  if (!address || !ADDRESS.test(address)) return json(res, 400, { success: false, error: { message: 'Invalid token address' } })
  const days = Math.min(Math.max(Number(daysRaw) || 1, 1), 365), marketAddress = address.toLowerCase() === ZERO ? WETH : address
  try {
    const [token, pools] = await Promise.all([gt(`/networks/eth/tokens/${marketAddress}`), gt(`/networks/eth/tokens/${marketAddress}/pools?page=1`)] )
    const attrs = token?.data?.attributes || {}, topPool = pools?.data?.[0]
    let candles = []
    if (topPool?.attributes?.address) {
      const timeframe = days <= 1 ? 'hour' : days <= 30 ? 'hour' : 'day'
      const aggregate = days <= 1 ? 1 : days <= 7 ? 4 : days <= 30 ? 12 : 1
      const limit = Math.min(days <= 1 ? 24 : days <= 7 ? 42 : days <= 30 ? 60 : 365, 1000)
      const ohlcv = await gt(`/networks/eth/pools/${topPool.attributes.address}/ohlcv/${timeframe}?aggregate=${aggregate}&limit=${limit}&currency=usd&token=${marketAddress}`)
      candles = ohlcv?.data?.attributes?.ohlcv_list || []
    }
    const prices = candles.map(c => [Number(c[0]) * 1000, Number(c[4])]).filter(p => Number.isFinite(p[1])).reverse()
    const pvals = candles.map(c => ({ high: Number(c[2]), low: Number(c[3]), volume: Number(c[5]) })).filter(x => Number.isFinite(x.high))
    const latest = Number(attrs.price_usd)
    return json(res, 200, { success: true, data: {
      address, symbol: attrs.symbol || null, name: attrs.name || null, image: attrs.image_url || null, priceUsd: Number.isFinite(latest) ? latest : null,
      priceChange24h: Number(attrs.price_change_percentage?.h24), marketCapUsd: Number(attrs.market_cap_usd), fdvUsd: Number(attrs.fdv_usd), volume24hUsd: Number(attrs.volume_usd?.h24),
      high: pvals.length ? Math.max(...pvals.map(x => x.high)) : null, low: pvals.length ? Math.min(...pvals.map(x => x.low)) : null,
      prices,
    } })
...
async function tokenSearch(res, url) {
  ...
  if (!q) return json(res, 200, { success: true, data: [] })
  if (ADDRESS.test(q)) {
    try {
      const body = await gt(`/networks/eth/tokens/${q}/info`), a = body?.data?.attributes || {}
      return json(res, 200, { success: true, data: [{ address: q, symbol: a.symbol || short(q), name: a.name || 'ERC-20 token', image: a.image_url || null }] })
    } catch { return json(res, 200, { success: true, data: [{ address: q, symbol: short(q), name: 'ERC-20 token', image: null }] }) }
  }
  try {
    const body = await gt(`/search/pools?query=${encodeURIComponent(q)}&network=eth&page=1`)
    ...
    for (const pool of body?.data || []) {
      ...
      for (const rel of [pool?.relationships?.base_token?.data, pool?.relationships?.quote_token?.data]) {
        ...
        const token = included.find(x => x.type === 'token' && x.id === rel.id), a = token?.attributes || {}
        const address = rel.id.split('_').pop()
        if (!ADDRESS.test(address)) continue
        seen.add(rel.id); out.push({ address, symbol: a.symbol || short(address), name: a.name || 'Token', image: a.image_url || null })
...
async function serveFile(req, res, pathname) {
  const requestPath = pathname === '/' ? '/index.html' : pathname, safePath = normalize(requestPath).replace(/^(\.\.(\/|\\|$))+/, '')
  let filePath = join(ROOT, safePath)
  try {
    const info = await stat(filePath); if (info.isDirectory()) filePath = join(filePath, 'index.html')
    const data = await readFile(filePath)
    res.writeHead(200, securityHeaders({ 'Content-Type': MIME[extname(filePath)] || 'application/octet-stream', 'Cache-Control': filePath.endsWith('index.html') ? 'no-cache' : 'public, max-age=3600', 'Content-Length': data.length })); res.end(data)
  } catch {
    try { const data = await readFile(join(ROOT, 'index.html')); res.writeHead(200, securityHeaders({ 'Content-Type': MIME['.html'], 'Cache-Control': 'no-cache' })); res.end(data) }
